...
import { ArrowLeft, ArrowRight, Building2, Lock, Mail, Store, Truck, User } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleSendVerificationCode = async () => {
    ...
    try {
      ...
      toast({
        title: '发送失败',
        description: error instanceof ApiError ? error.message : '验证码发送失败，请稍后重试',
        variant: 'destructive',
      });
    } finally {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: '注册成功',
        description: role === 'vendor' ? '欢迎加入供应商平台！' : '欢迎加入跨境电商平台！',
      });
      ...
      toast({
        title: '注册失败',
        description: error instanceof ApiError ? error.message : '该邮箱已被注册或服务器错误',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const nextStep = () => {
    ...
    if (step === 'info') {
      ...
      if (role === 'seller' && !validateStoreName(companyName)) {
        toast({
          title: '店铺名称格式错误',
          description: '店铺名称必须包含英文字母，只能使用英文字母、数字和连字符',
          variant: 'destructive',
        });
        return;
      }
        ...
        <div className="mx-auto w-full max-w-md space-y-6 lg:max-w-none">
          <div className="flex flex-col items-center space-y-2 lg:items-start">
            ...
            <p className="text-sm text-muted-foreground">
              {role === 'vendor' ? '让您的优质产品触达全球买家' : '开启您的跨境电商创业之旅'}
            </p>
          </div>
                      ...
                      >
                        <div>
                          ...
                          >
                            ...
                            <div className="text-center">
                              <p className="text-base font-semibold">供应商</p>
                        ...
                        <div>
                          ...
                          >
                            ...
                            <div className="text-center">
                              <p className="text-base font-semibold">采购商</p>
                              <p className="mt-1 text-sm text-muted-foreground">我要开启跨境电商</p>
                            </div>
                            <div className="mt-3 text-xs text-muted-foreground">
                              <ul className="space-y-2">
                                <li className="flex items-center gap-2">
