...
module.exports = {
  ...
  transform: async (config, path) => {
    const locales = ['en', 'zh'];
    const basePath = locales.reduce((acc, locale) => acc.replace(`/${locale}`, ''), path) || '/';
    const currentLocale = locales.find(locale => path.startsWith(`/${locale}`)) || locales[0];

    return {
      loc: `${config.siteUrl}/${currentLocale}${basePath === '/' ? '' : basePath}`,
      changefreq: config.changefreq,
      priority: config.priority,
      lastmod: new Date().toISOString(),
